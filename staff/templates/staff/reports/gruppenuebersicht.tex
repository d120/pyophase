...
{% for group, rooms in grouprooms %}
	\Gruppe{ {{ group.name|tex_escape }} }{% templatetag openbrace %}{{ group.get_picture_name }}{% templatetag closebrace%}{ {% for tutor in group.tutors.all %}\Tutor{ {{ tutor.get_name }} }{ {{ tutor.phone }}}{% endfor %} }{ {% for room in rooms %}\Raum{ {{ room|tex_escape }} } {% endfor %} }
		{% endfor %}
\end{longtable}
\end{document}
{% endautoescape %}
